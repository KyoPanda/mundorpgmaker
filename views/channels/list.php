...
<li class='depth<?php echo $channel["depth"]; ?><?php if ($channel["lft"] + 1 < $channel["rgt"]): ?> hasChildren<?php endif; ?>' id='channel-<?php echo $channel["channelId"]; ?>'>
    ...
    <div class='stats subText'>
        <span>
            <?php echo Ts("%s conversation", "%s conversations", $channel["countConversations"]); ?>
        </span>
    </div>

    <div class='info'>
        <a href='<?php echo URL("conversations/".$channel["slug"]); ?>' class='channel channel-<?php echo $channel["channelId"]; ?>'>
            <?php echo $channel["title"]; ?>
            <?php if ($channel['hasUnapproved']): ?>
                <span class='unapprovedMessages'>(!)</span>
            <?php endif; ?>
        </a>

        <?php if (!empty($channel["description"])): ?>
            <p class='description'><?php echo $channel["description"]; ?></p>
        <?php endif; ?>
    </div>
</li>
